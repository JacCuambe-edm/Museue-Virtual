...
const PatrimonioComercial: React.FC = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
         const data = await api.getArticles({ category: 'Comercialização' });
        setArticles(data);
      } catch (error) {
          ...
          <div className="bg-white rounded-lg shadow-2xl p-8 md:p-10">
            <div className="border-l-4 border-brand-orange pl-6">
              <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">
                Patrimônio Comercialização
              </h1>
              <p className="text-gray-600 leading-relaxed text-left md:text-justify">
                Explore a história comercial da EDM e os espaços que marcaram a relação com os clientes moçambicanos ao longo das décadas de serviço.
              </p>
            </div>
          </div>
        </div>
...
export default PatrimonioComercial;
