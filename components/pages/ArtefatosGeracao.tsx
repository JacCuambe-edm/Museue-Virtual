...
const ArtefatosGeracao: React.FC = () => {
  ...
  useEffect(() => {
    const fetchArtifacts = async () => {
      try {
        // Fetch all and filter client-side to avoid encoding issues with 'Geração'
        const all = await api.getArtefatos();
        const filtered = all.filter((a: any) => a.area_negocio === 'Geração');
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white p-8 rounded-xl shadow-lg border-b-4 border-orange-500 text-center">
            ...
            <div className="text-3xl font-bold text-slate-900 mb-1">
              {artifacts.filter(a => a.titulo.toLowerCase().includes('turbina')).length}
            </div>
          ...
          <div className="bg-white p-8 rounded-xl shadow-lg border-b-4 border-orange-500 text-center">
            ...
            <div className="text-3xl font-bold text-slate-900 mb-1">
              {artifacts.filter(a => a.titulo.toLowerCase().includes('gerador')).length}
            </div>
              ...
              >
                <div className="relative h-48 overflow-hidden">
                  <img 
                    ...
                    onError={(e) => {
                      e.currentTarget.src = 'https://via.placeholder.com/400x300/f97316/ffffff?text=EDM+Geração';
                    }}
                  />
